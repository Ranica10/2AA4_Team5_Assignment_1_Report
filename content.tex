...
\section{Requirements traceability}
    ...
    \hline
    \textbf{Req ID} & \textbf{Status} & \textbf{File} & \textbf{Design considerations} \\
    \hline
    \endfirsthead

    
    R1.1 & Implemented & \code{Board.java}, \code{Tile.java}, \code{Node.java} & The software shall first set up a valid map. Use the specified identification mechanism for tiles and nodes. \\
    \hline
    ...
    \hline
    R1.3 & Implemented & \code{Game.java}, \code{Board.java} & The simulation must follow the rules outlined in the attached rulebook. (Minus some features and feature groups listed below, after the requirements.) \\
    \hline
    ...
    \hline
    R1.6 & Implemented & \code{Board.java}, \code{Node.java}, \code{Tile.java} & The key invariants of the game must be respected, including: new roads must be connected to existing ones or roads or settlements; newly built cities must replace existing settlements; the distance between different players' settlements/cities must be at least two; etc. \\
    \hline
...
The following UML diagram represents the structure of our game system, and it shows the main classes, and the relationships between them.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{images/uml.png}
...
During implementation, we did not get the domain model completely right on the first attempt. We had to iterate between modelling and implementation several times. For example, in our initial UML, the takeTurn() method was placed inside the Player class. However, during implementation, we realized that turn logic required coordination between multiple components such as Board, Tile, Node, and Dice. Because of this, we refactored the design and moved takeTurn() to the Board class, where it made sense to manage the overall game flow. To improve readability, maintainability, and follow good design practices, we separated the takeTurn() method into several helper methods: produceResource(), buildRoad(), buildSettlement(), buildCity(), isRoadConnected(), and addRoadToBoard(). This separation allowed each method to focus on a single task, making the code easier to understand and modify. 

Another key refinement was adding the adjacentNodes list to the Node class. Initially, our model did not explicitly account for adjacency between nodes. During implementation, we ran into issues enforcing the rule that settlements must be at least two nodes apart. By introducing the adjacency mechanism and connecting nodes properly in the Board class, we were able to check distances and validate settlement placements effectively. This addition also helped in building roads correctly, as connectivity could now be verified using the adjacency information. We also added more functionality in the Board class to connect Tile and Node objects, which was not fully specified in the original UML. Similarly, the Player class gained helper methods such as addResource() and addBuilding(), which were necessary to make the program work properly but were not detailed in the UML. 

Overall, modelling and implementation operate at different levels of abstraction. UML focuses on high-level relationships and responsibilities, while coding requires attention to the details of object interactions, data structures, and constraints. This experience showed us that initial designs are rarely perfect; refining the model through iterative implementation is a natural and essential part of software development. It also highlighted that some implementation details, like node adjacency or helper methods, are often discovered only when translating abstract models into working code.
...
\textbf{How did you divide the workload?}
\newline
We divided the workload using a Kanban board in order to effectively manage tasks and keep everyone’s contributions balanced. The project was broken down into smaller tasks by class (e.g. Player, Board, Buildings, Resources, etc.), and each task was added to the shared board. Through this method, it allowed everyone to work across multiple parts of the system instead of one person handling it all or having merge conflicts by another member working on the same file. As a result, responsibility was distributed evenly and shared among the team.

\textbf{How much time did each team member work on the project?}
\newline
We aimed to achieve a roughly equal distribution of time commitment from all members. Because we used a Kanban board and split the work into tasks, everyone continuously picked up tasks throughout the development process as needed/brought up instead of one person completing a large section alone. As a result, each team member spent approximately the same amount of time working on the project.
...
\newline
To handle the time pressure, we distributed the workload among all team members so that the tasks were evenly divided. This ensured that the burden of completing all the tasks was not on one person and that multiple aspects of the project could be worked on simultaneously. Additionally, we handled the time pressure by starting the project as soon as the assignment details were released. This not only gave us time to clarify requirements early, but also reduced rushing near the deadline. As we knew we had a multitude of assignments from other courses, we worked consistently throughout the timeline of the project and kept steady progress, which prevented any last minute panic.
...
\begin{itemize}
    \item {\textbf{Raadhikka Gupta} contributed to the UML majorly, generating code using Papyrus, Game and Tile classes, and the report.}
    \item \textbf{Ranica Chawla} contributed to the UML diagram, generating code via Papyrus, Board, Node, Resources, Tile, GameLogger, Dice, MultiDice, and RegularDice classes, and the report.
	\item {\textbf{Nitya Patel} contributed to the UML diagram, generating Board, Player, Node, Demonstrator classes and the report.}
	\item {\textbf{Krisha Patel} contributed to initial UML planning, generating code from GenAI, Building, City, Settlement classes, and the report.}
\end{itemize}
